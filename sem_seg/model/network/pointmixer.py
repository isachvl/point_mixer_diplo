...
class PointMixerBlock(nn.Module):
    """Базовый residual-блок PointMixer.

    Внутри блока точки сначала смешиваются с ближайшими соседями
    (IntraSet/InterSet), а затем результат прибавляется к исходным признакам.
    """
    expansion = 1
...
class PointMixerSegNet(nn.Module):
    """Основная архитектура PointMixer для семантической сегментации.

    Encoder постепенно сжимает облако точек и увеличивает число признаков,
    decoder возвращает признаки обратно к исходным точкам, а cls-голова
    ...
    def __init__(
        ...
        # Encoder: последовательно уменьшаем число точек (N -> N/256)
        # и собираем более крупный контекст сцены.
        ...
        # Decoder: поднимаем признаки обратно к плотности исходных точек
        # и объединяем глубокий контекст с локальными деталями.
        ...
        # Semantic head: logits классов для каждой точки.
        self.cls = nn.Sequential(
            nn.Linear(planes[0], planes[0]), 
            nn.BatchNorm1d(planes[0]), 
            nn.ReLU(inplace=True), 
            nn.Linear(planes[0], k))
        self.out_planes = planes[0]
    ...
    def forward_features(self, pxo):
        """Возвращает признаки каждой точки после encoder-decoder PointMixer."""
    ...
    def forward(self, pxo):
        # Обычный PointMixer выдает только семантические классы точек.
        x = self.forward_features(pxo)
        return self.cls(x)


class PointMixerPanopticNet(PointMixerSegNet):
    """Паноптическая версия PointMixer.

    Наследует backbone и semantic head от PointMixerSegNet, но добавляет
    offset-голову: для каждой точки модель пытается предсказать сдвиг к
    центру своего объекта. Дальше на inference эти сдвинутые точки можно
    кластеризовать и получить instance_id.
    """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Offset head: 3 числа на точку (dx, dy, dz) к центру объекта.
        # Это та часть, которая делает архитектуру пригодной для паноптики.
        self.offset = nn.Sequential(
            nn.Linear(self.out_planes, self.out_planes),
            nn.BatchNorm1d(self.out_planes),
            nn.ReLU(inplace=True),
            nn.Linear(self.out_planes, 3))

    def forward(self, pxo):
        # Общий PointMixer backbone один и тот же для semantic и offset heads.
        feat = self.forward_features(pxo)
        return {
            # "Что это за точка?" - logits классов.
            'semantic_logits': self.cls(feat),
            # "К какому объекту она относится?" - сдвиг к центру instance.
            'pt_offsets': self.offset(feat),
        }
...
def getPointMixerPanopticNet(**kwargs):
    # Эту фабрику выбирает get_network.py, когда args.arch == 'pointmixer_panoptic'.
    model = PointMixerPanopticNet(PointMixerBlock, [2, 3, 4, 6, 3], **kwargs)
    return model
